...
function Lagrangien_Augmente(algo,fonc::Function,contrainte::Function,gradfonc::Function,
  ...
  while(true)
    ...
    if (norm(gradLagrangien(xk)) > epsilonk)
      if algo == "gct" || algo == "cauchy"
        xk,~ = Regions_De_Confiance(algo, lagrangien, gradLagrangien, hessLagrangien, xk, [])
      elseif algo == "newton"
        xk,~ = Algorithme_De_Newton(lagrangien,gradLagrangien,hessLagrangien,xk,[])
      else
        flag = -1
        break
      end
    ...
    if iter == itermax
      flag = 1
      break
    ##### CN1 ou x dans le domaine des contraintes : contrainte(x)=0 #####
    elseif norm(gradLNA(xk,lambdak)) <= max(epsilonk,tol*norm(gradLNA(x0,lambda0))) 
      flag = 0
      break
    elseif norm(contrainte(xk)) <= max(tol,tol*norm(contrainte(x0)))
      ...
      if (norm(contrainte(xk)) <= etak)
        ...
        lambdaks = vcat(lambdaks,[lambdak])
         ...
         #Mise à jour de parametre de pénalité  
        muk = tho*muk
