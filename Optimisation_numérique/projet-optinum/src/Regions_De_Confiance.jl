...
function Regions_De_Confiance(algo,f::Function,gradf::Function,hessf::Function,x0,options)
   ...
   while(true)
            nb_iters += 1
           #calcul du pas  sk qui réduit mk 
            if (algo=="cauchy")
            ...
            function m(s) 
                return f(xk) + gradf(xk)' * (s-xk) + (1 / 2) * (s - xk)' * hessf(xk) * (s - xk)
            end
            Pk = (f(xk) - f(xk + sk)) / (m(xk) - m(xk + sk))
            #Décroissance utilisée pour répondre aux questions
            decroissance =vcat(decroissance, (m(xk) - m(xk + sk)))
            if (Pk >= eta1)
                xk_1 = xk+sk
            ...
            elseif Pk < eta1 
                 ...
                 #### CN1#####
                 if (norm(gradf(xk_1))<max(Tol_rel*norm(gradf(x0)),Tol_abs))
                    flag=0
                    break
                
                #### Stagnation de x ######  
                 elseif (norm(sk)<epsilon*max(Tol_rel*norm(xk),Tol_abs))
                       flag=1
                       break
                 
                #### Stagnation de f #####    
                 elseif  (norm(f(xk)-f(xk+sk))<epsilon*max(Tol_rel*norm(f(xk)),Tol_abs))   
                     flag=2
                     break
                 end
            xk=xk_1   
    end
     #### nombre d'itération maxiamle atteint##### 
